    ...
    return (
        <div className='md:p-10 p-6 w-full space-y-10 min-h-screen bg-background text-foreground'>
            ...
            <Card className=" max-w-4xl">
                <CardHeader>
                    <CardTitle className="text-xl text-foreground">Search User Orders</CardTitle>
                </CardHeader>
                <CardContent>
                    <form onSubmit={handleSearch} className="flex gap-4 items-center">
                        ...
                        />
                        <Button type="submit" disabled={loading} className="bg-orange-500 hover:bg-orange-600 text-foreground font-bold w-32">
            ...
            <div className="max-w-4xl space-y-4">
                ...
                {!loading && searched && orders.length === 0 && (
                    <Card className=" text-center p-12">
                        ...
                        <p className="text-muted-foreground">No ticket orders found for this email address.</p>
                    </Card>
                )}

                {!loading && orders.length > 0 && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-bold text-foreground mb-4">Found {orders.length} order{orders.length === 1 ? '' : 's'}</h3>
                        {orders.map((order) => (
                            <Card key={order._id} className=" overflow-hidden ">
                                <div className="border-b border-border bg-muted/30 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="flex items-center gap-3">
                                        <FileText className="text-muted-foreground w-5 h-5" />
                                        <div>
                                            <p className="text-xs text-muted-foreground uppercase font-black tracking-wider">Order Reference</p>
                                            <p className="font-mono text-sm tracking-widest text-foreground">{order.reference}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-3 text-xs font-bold uppercase tracking-wider">
                                        <div className={`px-3 py-1.5 rounded-sm flex items-center gap-1.5 ${order.paymentStatus === 'success' ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>
                                ...
                                <CardContent className="p-6 flex flex-col md:flex-row gap-6">
                                    <div className="flex-1 space-y-4">
                                        <div>
                                            <h4 className="font-black text-xl mb-1 text-foreground">{order.event ? (order.event.title || `${order.event.homeTeam?.name || order.event.homeTeam} vs ${order.event.awayTeam?.name || order.event.awayTeam}`) : 'Event Deleted'}</h4>
                                            <p className="text-muted-foreground text-sm">Purchased on {new Date(order.createdAt).toLocaleDateString()} at {new Date(order.createdAt).toLocaleTimeString()}</p>
                                        </div>
                                        <div className="p-4 bg-muted/30 rounded-sm border border-border space-y-2">
                                            <p className="text-xs text-muted-foreground uppercase font-bold tracking-widest mb-2">Ticket Breakdown</p>
                                            ...
                                            <div className="border-t border-border pt-2 mt-2 flex justify-between font-black text-orange-500 text-sm">
                                                <span>Total Tickets</span>
                                                <span>{getTicketItems(order.tickets).reduce((sum: number, item: any) => sum + (Number(item.quantity) || 0), 0)}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="w-full md:w-64 space-y-4 border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6 flex flex-col justify-center">
                                       <div className="space-y-1">
                                            <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider">User Info</p>
                                            <p className="text-sm font-medium text-foreground">{order.user?.firstName} {order.user?.lastName}</p>
                                            <p className="text-xs text-muted-foreground truncate">{order.user?.email}</p>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider">Generation Status</p>
                                            {order.isGenerated ? (
                                                ...
                                                <div className="flex flex-col gap-2 mt-1">
                                                    ...
                                                    <Button 
                                                        ...
                                                        size="sm"
                                                        className="bg-orange-500 hover:bg-orange-600 text-foreground w-full font-black uppercase tracking-widest text-[10px] shadow-lg shadow-orange-500/20"
                                                    >
