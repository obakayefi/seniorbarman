        ...
        <div className="p-8 min-h-screen bg-background text-foreground">
            ...
            <div className="flex gap-4 mb-10 max-w-xl">
                <input
                    ...
                    className="flex-1 bg-muted border border-border p-3 rounded-sm text-foreground placeholder-zinc-600 focus:ring-2 focus:ring-orange-500 outline-none"
                    disabled={connected}
                />
                <button
                    onClick={() => setConnected(!connected)}
                    className={`px-8 py-3 rounded-sm font-bold transition-colors ${connected ? 'bg-red-500 hover:bg-red-600 text-foreground' : 'bg-orange-500 hover:bg-orange-600 text-foreground'}`}
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="border border-border p-6 rounded-sm bg-muted/50">
                    ...
                    {scans.length === 0 ? (
                        <p className="text-muted-foreground/70 italic text-sm py-4">Waiting for incoming check-ins...</p>
                    ) : (
                        <ul className="space-y-3">
                            {scans.map((scan, i) => (
                                <li key={i} className="p-4 rounded-sm border border-border bg-muted flex justify-between items-center">
                                    <div>
                                        <div className="font-bold">{scan.userName}</div>
                                        <div className="text-xs text-muted-foreground">{scan.time} • Stand: {scan.stand}</div>
                                    </div>
                ...
                <div className="border border-border p-6 rounded-sm bg-muted/50">
                    ...
                    {stats ? (
                        <pre className="text-xs text-orange-200/80 bg-background p-4 rounded-sm overflow-x-auto border border-border">
                            {JSON.stringify(stats, null, 2)}
                        </pre>
                    ) : (
                        <p className="text-muted-foreground/70 italic text-sm py-4">Waiting for stats payload update...</p>
                    )}
                </div>
            </div>
        </div>
