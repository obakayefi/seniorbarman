...
export default function TicketsForSalePage() {
    ...
    const drawTicketOverlays = async (ctx: CanvasRenderingContext2D, ticket: any, canvasWidth: number) => {
        const scale = canvasWidth / (isCompact ? 50 : 100) // pixels per mm

        if (isCompact) {
            // Compact Mode (50mm x 40mm) - Center QR code
            const qrSize = 25 * scale
            const x = (25 * scale) - (qrSize / 2)
            const y = (18 * scale) - (qrSize / 2)
            ...
            ctx.font = `900 ${2.5 * scale}px sans-serif`
            ctx.textAlign = "center"
            ctx.fillText(`#${ticket.ticketNumber.slice(-8).toUpperCase()}`, 25 * scale, 37 * scale)
            return;
        }
        ...
        // 4. Draw Holder Info (Side by side) - Adjusted for bottom position
        ctx.textAlign = "left"
        ctx.fillStyle = "#A1A1AA"
        ctx.font = `bold ${2 * scale}px sans-serif`
        ctx.fillText("HOLDER:", (13 * scale), (60 * scale))
        
        const labelWidth = ctx.measureText("HOLDER: ").width
        ctx.fillStyle = "#18181B"
        ctx.font = `900 ${2.5 * scale}px sans-serif`
        ctx.fillText(ticket.holderName || "Guest", (13 * scale) + labelWidth + (1 * scale), (60 * scale))
    }
    ...
    const generateZipFiles = async () => {
        ...
        try {
            ...
            if (downloadMode === 'individual') {
                ...
                const cols = isCompact ? 4 : 2
                const rows = isCompact ? 7 : 4
                const ticketsPerPage = cols * rows

                // Calculate px per mm (A4 is 210x297mm)
                const pxPerMm = a4Canvas.width / 210
                const ticketWidthPx = (isCompact ? 50 : 100) * pxPerMm
                const ticketHeightPx = (isCompact ? 40 : 65) * pxPerMm
        ...
        <div className="md:p-10 p-6 w-full space-y-8 min-h-screen bg-zinc-950 text-white pb-20">
            <div className="max-w-4xl mx-auto space-y-8">
                <div className="flex flex-col space-y-4">
                    <Link href={`/u/organizer/events/${id}`} className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors text-sm w-fit group">
                ...
                {!allTickets.length ? (
                        ...
                        <Card className="md:col-span-2 bg-zinc-900 border-zinc-800 shadow-xl text-white">
                            ...
                            <CardContent className="pt-6 space-y-6">
                                ...
                                <div className="space-y-8 pt-4 border-t border-zinc-800">
                                    <div className="space-y-3">
                                        <Label className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">1. Download Format</Label>
                                    ...
                                    <div className="space-y-3">
                                        <Label className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">2. Design Mode</Label>
                                        <div className="grid grid-cols-2 gap-4">
                                            <button 
                                                onClick={() => setIsCompact(false)}
                                                className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${!isCompact ? 'border-orange-500 bg-orange-500/10 text-white' : 'border-zinc-800 bg-zinc-900/50 text-zinc-500 hover:border-zinc-700'}`}
                                            >
                                                <div className="flex flex-col gap-1 w-full items-center">
                                                    <div className="w-8 h-1 bg-current opacity-40 rounded-full" />
                    ...
                    <div className="mt-12 space-y-4">
                        ...
                        <p className="text-sm text-zinc-400">See exactly how your tickets translate to physical paper ({isCompact ? '28' : '8'} tickets per page).</p>
                        <div className="bg-zinc-950 rounded-2xl border-2 border-dashed border-zinc-800 flex justify-center shadow-inner overflow-hidden h-[600px] relative">
                            <div className="scale-[0.50] origin-top absolute top-6 bg-white w-[210mm] h-[297mm] shadow-[0_0_50px_rgba(0,0,0,0.5)] shrink-0">
                                <div className={`absolute left-0 right-0 top-0 bottom-0 flex items-center justify-center`}>
                                    <div className={`grid ${isCompact ? 'grid-cols-4' : 'grid-cols-2'}`}>
                                        {allTickets.slice(0, isCompact ? 28 : 8).map(ticket => (
                                            <div key={ticket._id} className={`${isCompact ? 'w-[50mm] h-[40mm]' : 'w-[100mm] h-[65mm]'} border-[0.5pt] border-zinc-200 overflow-hidden`}>
                                                <TicketZipRenderer ticket={ticket} templateMode={false} compactView={isCompact} />
                                            </div>
                                        ))}
                                    </div>
                                </div>
